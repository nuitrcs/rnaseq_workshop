...
2. (Optional) For running analysis on the compute nodes, it is a good idea to [request access to the Genomic Compute Cluster on Quest](https://kb.northwestern.edu/page.php?id=78602).  *Steps 1-6 of this protocol are available as a submission script to run on Quest; RNAseq_workshop_submit.sh will run fastq, trimmomatic, hisat2 and stringtie on the compute nodes for the 6 samples used in this workshop.   While RNAseq_workshop_submit.sh defaults to running on the Genomic Compute Cluster (b1042), you can edit the script to work with your own allocation and the "short" queue if you'd prefer.  To run RNAseq_workshop_submit.sh on Quest, type the command: `msub RNAseq_workshop_submit.sh` after you have copied the workshop directory (5.)*
